...
export function LabSideNav({ active = 'HOME' }) {
  ...
  return <aside className="lab-side-nav">{items.map((item, i) => <Link key={item.label} to={item.href} className={item.label === active ? 'active' : ''}><span>{String(i + 1).padStart(2, '0')}</span><b>{item.label}</b></Link>)}</aside>;
}
