...
function Hero() {
  return <section className="lab-hero"><img className="lab-hero-bg" src={labImages.hero} alt="航天仿真研究主视觉"/><div className="lab-hero-shade"/><div className="lab-hero-content"><div className="lab-hero-copy"><div className="lab-kicker">AEROSPACE SIMULATION RESEARCH</div><h1>仿真。<br/>分析。<br/>探索。</h1><p>面向中文用户的航天仿真研究控制台：汇集工程软件、开源项目、公开数据、任务案例、参数推断方法与复现实验记录。</p><div className="lab-hero-actions"><Link to="/intelligence">开始探索 ↗</Link><span>向下滚动查看研究控制台</span></div></div><div className="lab-status-panel">{missionStats.map(item => <div key={item.label}><span>{item.label}</span><strong>{item.value}</strong></div>)}<svg viewBox="0 0 220 44" aria-hidden="true"><path d="M2 28 C45 5, 80 42, 115 24 S180 16, 218 6"/><circle cx="55" cy="19" r="4"/><circle cx="118" cy="25" r="4"/><circle cx="178" cy="13" r="4"/></svg></div></div></section>;
}

function ConsoleBand() {
  return <section className="lab-console-band"><div className="lab-update-card"><span>最新更新</span><div><img src={labImages.data} alt="最新研究更新"/><article><em>2024-05-25</em><h3>重构航天仿真研究控制台视觉系统</h3><p>ENGINE · SIMULATION</p></article></div></div><div className="lab-digest"><span>每日研究简报</span>{latestUpdates.map(item => <div key={item.title}><b>{item.title}</b><em>{item.date}</em></div>)}<Link to="/intelligence">查看全部情报 ↗</Link></div><div className="lab-quick"><span>快速入口</span><div>{quickAccess.map(item => <Link key={item.title} to={item.href}><b>{item.cn}</b><em>{item.title}</em></Link>)}</div></div><div className="lab-orbit-widget"><div className="lab-orbit-rings"><i/><i/><i/><button>▶</button></div></div></section>;
}
