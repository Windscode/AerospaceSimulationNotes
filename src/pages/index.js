...
function Img({ src, alt }) { return <img src={useBaseUrl(src)} alt={alt} loading="lazy" />; }
    ...
    <Layout title="首页" description="航天仿真工程软件、开源项目、论文资料与复现实验的长期知识门户">
      <main className="asn-page">
        <section className="asn-hero">
          <img className="asn-hero__image" src={useBaseUrl(visualAssets.hero)} alt="航天仿真任务控制与轨道数据可视化主视觉" />
          <div className="container asn-hero__content">
            <div>
              <div className="asn-badge">航天仿真研究门户</div>
              <h1>不是收藏夹，而是可持续维护的航天仿真知识资产。</h1>
        ...
        <section className="asn-section container">
          <SectionTitle label="核心定位" title="工程软件 + 开源项目 + 研究情报 + 复现实验">首页优先展示真正有用的入口，而不是普通博客目录。每天新增资料时，先进入研究情报，再沉淀到软件栈、项目雷达、论文笔记和实验记录。</SectionTitle>
          <div className="asn-feature"><Img src={visualAssets.tools} alt="工程软件生态地图"/><div className="asn-feature__body"><div className="asn-kicker">Engineering Software Landscape</div><h2>把 MATLAB、Ansys、STK、GMAT、Orekit 等工具放进同一个工程语境。</h2><p>每个工具都记录用途、成熟度、适用领域、优先级、替代方案和学习价值，避免只停留在“知道有这个软件”的层面。</p><div className="asn-actions"><Link className="asn-button asn-button--primary" to="/tools">查看工具矩阵</Link></div></div></div>
        </section>
        ...
        <section className="asn-section container">
          ...
          <div className="asn-grid asn-grid--3">{domains.map((d) => <Link className="asn-card" to={d.link} key={d.title}><Img src={d.image} alt={d.title}/><div className="asn-kicker">Knowledge Domain</div><h3>{d.title}</h3><p>{d.desc}</p></Link>)}</div>
        </section>
