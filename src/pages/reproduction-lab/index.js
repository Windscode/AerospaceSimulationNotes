  ...
  return <Layout title="复现实验室" description="航天仿真复现实验、验证和归档流程">
    <AeroLabFrame active="LAB">
      ...
      <section className="lab-page-section">
        <div className="lab-page-head"><div><span>实验控制台</span><h2>实验工作台</h2></div><p>先让用户看到实验场景，再进入门禁、证据链、参数推断和任务案例。</p></div>
      ...
      <section className="lab-page-section">
        <div className="lab-page-head"><div><span>验证门禁</span><h2>六个门禁</h2></div><p>没有来源、输入、输出、误差和适用范围，就不能成为稳定知识。</p></div>
      ...
      <section className="lab-page-section">
        <div className="lab-overlay-panel"><img src={labImages.hero} alt="任务复现实验"/><div><span>任务驱动验证</span><h3>围绕任务对象验证。</h3><p>后续每个案例都会把工具链、公开数据、反推参数、运行脚本和可视化结果连接起来，形成真正可维护的航天仿真研究资产。</p></div></div>
      </section>
      <section className="lab-page-section">
        <div className="lab-page-head"><div><span>任务案例</span><h2>候选任务</h2></div><p>这些任务案例后续会逐步扩展为完整实验记录。</p></div>
      ...
      <section className="lab-page-section">
        <div className="lab-page-head"><div><span>证据链</span><h2>归档证据链</h2></div><p>每次复现都要能回答：跑了什么、为什么可信、误差在哪里、下一步怎么改。</p></div>
