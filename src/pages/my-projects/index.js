...
import { labImages, missionDossiers, featuredProjects } from '../../data/aerolabContent';
import { lifecycle, validationGates } from '../../data/siteContent';
...
export default function MyProjectsPage(){
  return <Layout title="我的项目" description="个人航天仿真项目、开发日志和复现实验路线">
    <AeroLabFrame active="MINE">
      <LabPageHero eyebrow="PERSONAL PROJECTS · 我的项目" title="我的项目" text="这里不是资料目录，而是个人正在推进的航天仿真研究和开发主线：网站建设、工具链实验、任务案例、复现实验和长期路线图。" image={labImages.control} stats={[{label:'项目类型', value:'研究 / 开发'}, {label:'状态', value:'持续推进'}, {label:'沉淀方式', value:'日志 + 实验'}]} />
      ...
      <section className="lab-page-section">
        <div className="lab-page-head"><div><span>候选实验</span><h2>任务实验池</h2></div><p>这些任务会逐步变成可运行、可验证、可归档的实验记录。</p></div>
      ...
      <section className="lab-page-section">
        ...
        <div className="lab-status-grid">{validationGates.map((g, i) => <article key={g.title}><span>门禁 {String(i + 1).padStart(2,'0')}</span><strong>{g.title}</strong><p>{g.desc}</p></article>)}</div>
      </section>
      <section className="lab-page-section">
      ...
      <section className="lab-page-section">
        <div className="lab-page-head"><div><span>流转记录</span><h2>从发现到归档</h2></div><p>每个项目都按统一生命周期推进，方便后续每天维护。</p></div>
