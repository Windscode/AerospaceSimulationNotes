...
export default function RadarPage() {
  ...
  return <Layout title="开源项目评估" description="航天仿真开源项目评估与复现状态">
    <AeroLabFrame active="OPEN">
      <LabPageHero eyebrow="OPEN SOURCE RADAR · 内部模块" title="项目评估" text="这是“开源与数据”下的内部模块。开源项目不是收藏夹条目，而要判断用途、成熟度、许可证、复现难度、工程价值和任务适配关系。" image={labImages.orbit} stats={[{label:'A 级项目', value:String(projects.filter(p=>p.rating==='A').length)}, {label:'领域数量', value:String(domains.length - 1)}, {label:'状态', value:'筛选中'}]} />
      <section className="lab-page-section">
        <div className="lab-page-head"><div><span>项目侦察</span><h2>候选项目</h2></div><p>先用任务场景解释为什么跟踪这些项目，再把详细矩阵留给后续维护。</p></div>
      ...
      <section className="lab-page-section">
        <div className="lab-page-head"><div><span>任务适配</span><h2>适配方向</h2></div><p>开源项目只有能支撑轨道传播、GNC 闭环、CFD 建库、遥测显示或任务回放，才值得长期跟踪。</p></div>
      ...
      <section className="lab-page-section">
        <div className="lab-overlay-panel"><img src={labImages.data} alt="开源项目评估"/><div><span>评估矩阵</span><h3>用途、复现、许可证。</h3><p>每个开源项目都要回答：能解决什么仿真问题，是否能复现，许可证是否清楚，是否值得作为工具链基准，是否能支撑某个任务案例。</p></div></div>
      </section>
      <section className="lab-page-section">
        <div className="lab-page-head"><div><span>项目矩阵</span><h2>评估矩阵</h2></div><p>这里不追求条目数量，先把项目用途、复现状态和工程价值写清楚。</p></div>
